...
class MolGraph(object):
    ...
    def neighbor_list(self, self_ntype, neighbor_ntype):
        ...
        assert neighbor_ntype in self.nodes
        assert self_ntype in self.nodes
...
def graph_from_smiles(str_smiles, fp_switch): #ecfp = false, fcfp = true
    graph = MolGraph()
    '''
    check = np.array(1)
