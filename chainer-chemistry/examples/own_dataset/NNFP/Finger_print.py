...
from .mol_graph import MolGraph, graph_from_smiles_tuple, graph_from_smiles, degrees
...
def sum_and_stack(features, slice_size):
	batch_size = int(len(features)/slice_size)
	features = np.reshape(features,(batch_size,slice_size,len(features[0])))	
	sum_list = np.empty((features[0][0].shape))
	for batch in range(batch_size):
		sum_list = np.vstack((sum_list,features[0][0]*0))
		for idx in range(slice_size):
			sum_list[batch+1] += features[batch][idx]	
	sum_list = np.delete(sum_list,0,0)
	return sum_list
...
def array_rep_from_smiles(smiles, fp_switch):
    ...
    #print ("smiles : ", (smiles))
    molgraph = graph_from_smiles(smiles, fp_switch)
    #print (molgraph.feature_array('atom'))
    #print ("array rep from smile debug")
    ...
    '''
    #print ("array rep from smile debug", arrayrep)
    #print (arrayrep)
    return arrayrep


def matmult_neighbors(self, array_rep, adj, degree_num, get_weights_func): 
...
class ECFP(Chain): #fp_switch: ecfp is False
	...
	def __call__(self, array_rep, adj, degree_num):
		#array_rep = array_rep_from_smiles(tuple(smiles), False) #rdkitで計算。smiles to data
	
		def update_layer(self, layer, atom_features, adj, normalize=False):
			...
			neighbor_activations = matmult_neighbors(self,
				atom_features, adj, degree_num,  get_weights_func)
		...
		def output_layer_fun_and_atom_activations(self, array_rep, adj):
			...
			def write_to_fingerprint(self, atom_features, layer):
				...
				atom_slice = int(atom_features.shape[0] / self.model_params['batch_size'])
				layer_output = sum_and_stack(atom_outputs, atom_slice)
				all_layer_fps.append(layer_output)

			num_layers = self.model_params['fp_depth']
			for layer in range(num_layers):
				write_to_fingerprint(self, array_rep, layer)
				array_rep = update_layer(self, layer, array_rep, adj, normalize=False)
				#atom_features = atom_features._data[0]

			write_to_fingerprint(self, array_rep, num_layers)
			return sum(all_layer_fps), atom_activations, array_rep
	
		def output_layer_fun(self, array_rep, adj):
			output, _, _ = output_layer_fun_and_atom_activations(self, array_rep, adj)
			return output
	
		def compute_atom_activations(self, array_rep, adj):
			_, atom_activations, array_rep = output_layer_fun_and_atom_activations(array_rep, adj)
			return atom_activations, array_rep
		conv_fp_func = output_layer_fun
		return (conv_fp_func(self, array_rep, adj))
...
class FCFP(Chain): #fp_switch: fcfp is True
	...
	def __call__(self, array_rep, adj ,degree_num):
		...
		def compute_atom_activations(self, array_rep, adj):
			_, atom_activations, array_rep = output_layer_fun_and_atom_activations(arrey_rep, adj)
			return atom_activations, array_rep
		conv_fp_func = output_layer_fun
		return (conv_fp_func(self, array_rep, adj))
