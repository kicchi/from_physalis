...
task_params = {'target_name' : 'log(1/LC50)',
               'data_file'   : 'FHMDB_need_log_error0.csv'}
N_train = 700
N_val   = 20
N_test  = 20
...
def main():
    ...
    traindata, valdata, testdata = load_data(
        ...
        input_name='SMILES,C,100', target_name=task_params['target_name'])
    train_inputs, train_targets = traindata
    val_inputs,   val_targets   = valdata
    test_inputs,  test_targets  = testdata

    print "data vari"
    ave = sum(train_targets) / len(train_targets)
    var = 0
    for data in train_targets:
	    var += (data - ave) * (data - ave)	
    var /= len(train_targets)
    print var
