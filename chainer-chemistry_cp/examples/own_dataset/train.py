...
from chainer_chemistry.models import MLP, NFP, GGNN, SchNet, WeaveNet, RSGCN, Regressor, NFP_attention  # NOQA
...
class GraphConvPredictor(chainer.Chain):
    ...
    def __call__(self, atom1, atom2, adjs, degree_num):
        x = self.graph_conv(atom1, atom2, adjs, degree_num)
        if self.mlp:
            x = self.mlp(x)
        return x
...
def main():
    ...
    #データファイルは--datafileで設定, smilesでおっけー！！
    ...
    #print ("IN train.py : dataset : ",dataset)
    ...
    if method == 'nfp':
        ...
        print('Train NFP_attention model...')
        model_params = dict(fp_length = n_unit,
                            fp_depth = conv_layers,
                            conv_width = n_unit,
                            h1_size =100,
                            L2_reg = numpy.exp(-2),
                            batch_size = args.batchsize)
        model = GraphConvPredictor(
            NFP_attention(out_dim=n_unit, hidden_dim=n_unit, n_layers=conv_layers,model_params=model_params),
            MLP(out_dim=class_num, hidden_dim=n_unit))
    else:
    ...
    degree_num = preprocessor.get_degree()
    atoms1, atoms2, adj = concat_mols([input_features], device=args.gpu)
    #print ("atoms : ",atoms)
    #print ("adjs : ",adjs)
    prediction = model(atoms1,atoms2, adjs, degree_num).data[0]
